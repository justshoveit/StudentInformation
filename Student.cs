...
namespace CourseWork
{
    public class Student
    {
        ...
        public DateTime[] week(DateTime inputDate)
        {
            DateTime[] arraydate = new DateTime[2];
            string[] arr = new string[] { "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturyday" };
            int ind = Array.IndexOf(arr, inputDate.DayOfWeek.ToString());
            DateTime dateStart = inputDate.AddDays(-ind);
            int indexremain = 6 - ind;
            DateTime dateEnd = inputDate.AddDays(indexremain);
            arraydate[0] = dateStart;
            arraydate[1] = dateEnd;
            return arraydate;
        }
        public List<Student> week(DateTime[] datearray, List<Student> listStudents)
        {
            List<Student> report = new List<Student>();
            for (int i = 0; i < listStudents.Count(); i++)
            {
                if (listStudents[i].EnrollDate > datearray[0] && listStudents[i].EnrollDate < datearray[1])
                {
                    report.Add(listStudents[i]);
                }
            }
            return report;
        }
